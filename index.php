...
$json_data_file_name = 'short_urls.json';

header('X-Server: ClickIt-URL-Shortener, by Vino Rodrigues (@vinorodrigues)');
header('X-Server-Source: https://github.com/vinorodrigues/clickit-url-shortener');
...
if (('u' == $command) && empty($promise) && file_exists($json_data_file_name) ) {
...
if (('u' == $command) || ('@' == $command)) {
  ...
  // Lets go! ... first load the data

  $urls = false;
  if (file_exists($json_data_file_name)) {
    $urls = @json_decode( file_get_contents($json_data_file_name), true );
    if ((null == $urls) || (JSON_ERROR_NONE !== json_last_error()) ) {
      $command = 'e';
      $content = '<p>JSON error code <b>' . json_last_error() . '</b> in file <code>' . $json_data_file_name . '</code></p>';
      $promise = 500;
      $urls = false;
    }
  } else {
  ...
  if (false !== $urls) {
    if (isset($urls['urls'])) $urls = array_change_key_case($urls['urls'], CASE_LOWER);  // sink 1 layer
...
switch ($command) {
  ...
  case '-':
    ...
    $content = '<p class="">Short URL = <code>' . $short . '</code><br>' .
      ...
      '<p class="text-center"><img src="' . generateQRCodeURL($url, $qr_code_engine) . '" class="img-fluid img-thumbnail"></p>' .
  ...
  case '@':
    // --- QR-code ---
    $url = generateQRCodeURL($url, $qr_code_engine);
  ...
  case 'h':
    ...
    $inc_fa = true;
    $content = '<div class="text-center">My name is <tt><b>' . $_SERVER['SERVER_NAME'] . '</b></tt>' .
      '<span class="text-muted">,<br>and I\'m running code from the' .
      ' <a href="https://github.com/vinorodrigues/clickit-url-shortener"><i class="fa-brands fa-github-alt"></i> ClickIt-URL-Shortener</a> project.</span>';
    break;
    ...
    case 'i':
      ...
      if (($data != null) && (JSON_ERROR_NONE === json_last_error()) ) {
        if (array_key_exists($promise, $data)) {

          $ct = isset($data[$promise]['c']) ? $data[$promise]['c'] : 'text/plain';

          if (isset($data[$promise]['d']) && !empty(isset($data[$promise]['d']))) {
            if (isset($data[$promise]['b']) && $data[$promise]['b']) {
              $content = base64_decode( $data[$promise]['d'] );
            } else {
              $content = $data[$promise]['d'];
            }
          } else {
            $content = '';
          }

          $hold = isset($data[$promise]['h']) ? intval($data[$promise]['h']) : DEFAULT_CACHE_TIME;
        ...
        $content = 'JSON ERROR: ' . json_last_error();
        $promise = 500;
      }
  ...
  case 'x':
    ...
    $content = '<p class="card-text">File <code>' . $json_data_file_name . '</code> not found.</p>
...
<head>
  ...
  <link href="https://cdn.jsdelivr.net/gh/vinorodrigues/bootstrap-dark@0.6.1/dist/bootstrap-dark.min.css" rel="stylesheet" media="all" integrity="sha384-GrGBtUiVQd0B0YKF+fKYa7+UY5kc/9N8AvsG1+zcHqslLuITk8fKygUaYv6TnGQ9" crossorigin="anonymous">
<?php if ($inc_fa) { ?>
  <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css" rel="stylesheet" media="all" integrity="sha384-PPIZEGYM1v8zp5Py7UjFb79S58UeqCL9pYVnVPURKEqvioPROaVAJKKLzvH2rDnI" crossorigin="anonymous">
<?php } ?>
<?php if ($inc_highlighter) { ?>
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/styles/default.min.css" integrity="sha384-4Y0nObtF3CbKnh+lpzmAVdAMtQXl+ganWiiv73RcGVdRdfVIya8Cao1C8ZsVRRDz" crossorigin="anonymous">
<?php } ?>
  ...
  <style>
    @media (prefers-color-scheme: light) {
      body { background-image: linear-gradient(to right, #2c3e50, #4ca1af); }
    }
    @media (prefers-color-scheme: dark) {
      body { background-image: linear-gradient(to right, #141e30, #243b55); }
    }
  </style>
</head>
<body>
